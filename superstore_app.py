...
st.header("Data Exploration and Cleaning Interface")
...
with col1:
    with st.expander("Show Dataset Head"):
        st.markdown("<h6 style='text-align: left; color: black;'>First 5 Rows of the Dataset</h6>", unsafe_allow_html=True)
        st.write(df.head().style.background_gradient(cmap="Blues"))
with col2:
    with st.expander("Show Dataset Tail"):
        st.markdown("<h6 style='text-align: left; color: black;'>Last 5 Rows of the Dataset</h6>", unsafe_allow_html=True)
        st.write(df.tail().style.applymap(lambda _: 'background-color: lightblue'))

col3, col4 = st.columns((2))
with col3:
    with st.expander("Show Dataset Info"):
        st.markdown("<h6 style='text-align: left; color: blue;'>Dataset Information</h6>", unsafe_allow_html=True)
        buffer = io.StringIO()
        df.info(buf=buffer)
        s = buffer.getvalue()
        st.text(s)
with col4:
    with st.expander("Show DataSet Shape"):
        st.markdown("<h6 style='text-align: left; color: blue;'>Dataset Shape</h6>", unsafe_allow_html=True)
        st.write(f"Rows: {df.shape[0]}, Columns: {df.shape[1]}")

col5, col6 = st.columns((2))
with col5:
    with st.expander("Show Dataset Columns"):
        st.markdown("<h6 style='text-align: left; color: red;'>Dataset Columns</h6>", unsafe_allow_html=True)
...
with col11:
    ...
    with st.expander("Find Outliers (Z-score)"):
        ...
        st.markdown("<h6 style='text-align: left; color: black;'>Outliers Detected using Z-score</h6>", unsafe_allow_html=True)
...
with col12:
    with st.expander("Find Outliers (IQR)"):
        ...
        st.markdown("<h6 style='text-align: left; color: black;'>Outliers Detected using IQR</h6>", unsafe_allow_html=True)
...
###############################
st.header("Analyzing Elements by Bar Charts")
col1, col2 = st.columns((2))
with col1:
...
with col:
    with st.expander("Sub_Category Count"):
         subcategory_count = filtered_df.groupby(by="Category")["Sub-Category"].nunique().reset_index(name='Sub-Category Count')
...
with col:
    with st.expander("TopSub-Category_ViewData"):
        # subcategory_df = numeric_df.groupby(by="Sub-Category", as_index=False)["Sales"].sum().sort_values(by="Sales", ascending=True)
        subcategory_df = filtered_df.groupby(by="Sub-Category", as_index = False)["Sales"].sum()
        st.write(subcategory_df.style.background_gradient(cmap="Blues"))
        csv = subcategory_df.to_csv(index=False).encode('utf-8')
        st.download_button("Download Data", data=csv, file_name="TopCategory.csv", mime="text/csv",key="download_button1",
                           help='Click here to download the data as a CSV file')
...
#sales %
st.subheader("Analyzing Elements By Pie Charts")
chart0 = st.columns((1))
chart = chart0[0]
with chart:
...
# profit and revenue relationship
